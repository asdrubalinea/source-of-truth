...
- `rices/{estradiol,niri}` — desktop environments. Each rice has a `system.nix` (imported by the host) and a `default.nix` / home-manager-side files (imported by the home config). `tempest` currently uses `niri`; `orchid` currently uses `estradiol`.
