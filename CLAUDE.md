...
- `update-home` (tempest only, `scripts/update-home.nix`) — `nix flake update` for the subset of inputs that only affect tempest's HM closure (`nixpkgs-home`, `claude-code`, `llm-agents`, `zen-browser`, `lights`, `emacs-overlay`, `stylix`, `hyprland`). `niri` and `helix` are intentionally excluded because both also live in tempest's system layer.
...
`ocelot` (`scripts/ocelot.{nix,sh}`, `hosts/ocelot/`, `homes/ocelot.nix`) is the
per-project dev VM — cage's sibling one layer down: a QEMU guest of one project
directory, entered over ssh, with its own docker daemon, that outlives the
connection. Unprivileged throughout (virtiofsd `--sandbox none` + passt + a
transient `systemd-run --user` unit; no libvirt). `ocelot` itself builds
`.#nixosConfigurations.ocelot` on every start, so it is the one thing in this
tree that legitimately runs `nix build` on a host config — it is not
`nixos-rebuild` and it activates nothing. **Do not confuse an ocelot with a host
clone**; `CONTEXT.md` (*Isolation (tempest)*) flags "the VM" as ambiguous here.
Mechanism and commands: `docs/ocelot.md`. Why: `docs/adr/0013`.
...
- `./build-vm <tempest|orchid>` — `nix build .#nixosConfigurations.<host>-vm.config.system.build.vmWithDisko`; run the result with `./result/bin/disko-vm`. (Do NOT use `nixos-rebuild build-vm` — it builds a generic VM that ignores the disko layout and the VM tuning in `hosts/<host>/vm.nix`.) `orchid-vm` runs on the serial console in the launching terminal (`virtualisation.graphics = false`, since the host is headless) and prints a boot report — `systemctl --failed` plus each failed unit's journal — to that console ~20s after `multi-user.target`, because the VM inherits root's hash from the git-crypt'd `passwords` file and so cannot be logged into.

Disk / install helpers are **destructive** — they wipe and reformat the target device. Only run when actually installing:

- `./disk-format <host> /dev/disk/by-id/<target>` — runs disko `destroy,format,mount` against `./disks/<host>.nix`. **Both** arguments are required (no defaults), and the layout files only carry a non-existent placeholder device, so a bare/accidental run fails fast instead of wiping a disk. Hosts with a parameterised layout: `tempest`, `orchid` (hydra hard-codes its device).
- `./disk-install <host> /dev/disk/by-id/<target>` — disko-install `.#<host>` to the given device (passed through as `--disk main`), then `zpool export rpool`, which is what lets `boot.zfs.forceImportRoot` stay `false`. Same required-argument safety. These two replaced the four `{tempest,orchid}-{format,install}` scripts, which differed only in the host name. Runbooks: `docs/tempest-zfs-install.md`, `docs/orchid-install.md`.
...
- `tempest-vm` — the same `hosts/tempest/default.nix` built with `virtual = true` (`mkTempest` in `flake.nix`), which drops the physical-hardware imports. Built via `./build-vm tempest`.
- `orchid-vm` — same idea for orchid (`mkOrchid`, `hosts/orchid/vm.nix`): there is no physical layer to drop, so `virtual = true` only adds guest sizing (4 GiB / 4 cores / 512G sparse image / serial console / swtpm), home-manager as a NixOS module, and `mkForce`-off for the units that need credentials a throwaway VM must not hold (borg, caddy, the bots, the vault export). Built via `./build-vm orchid`. Note the flake carries a `vmTools` overlay that both VMs depend on — disko passes `pkgs.aggregateModules` as vmTools' `kernel`, which current nixpkgs rejects for having no `target`; the overlay supplies `kernelImage`. Delete it when disko fixes this upstream.
- `orchid` — tower (Ryzen 7 7800X3D, 64 GiB, no dGPU). Rebuilt 2026-08-22 in tempest's shape: `disko` + `impermanence`, ZFS-on-LUKS with a tmpfs root (`disks/orchid.nix`), CachyOS LTS zen4 kernel, standalone Home Manager (`homeConfigurations."irene@orchid"`). **Headless for now** — no rice, no compositor, no greeter. No lanzaboote/secure boot and no `ucodenix` (unlike tempest). Not installed anywhere as of this writing; a reinstall is pending.
- `ocelot` — **not a machine.** The per-project dev VM's guest system, built and run only by `scripts/ocelot.sh`. Imports nixpkgs' `virtualisation/qemu-vm.nix` via `modulesPath`, shares the host store over virtiofs under a scratch overlay, and takes its per-project identity at boot from the launcher's staging share. Home Manager is attached as a NixOS module here (unlike tempest/orchid), because a guest rebuilt on every boot has no second activation step. See `docs/adr/0013`.
...
- `modules/` — cross-cutting system modules. `nix.nix` and `secure-boot.nix`, plus the three that exist because tempest and orchid converged on the same shape: `security.nix` (the doas + sudo-rs posture, imported by all four hosts — hydra/zephyr keep a two-line file for their extra `keepEnv` doas rule, tempest keeps its greetd keyring line in `system/session.nix`), `zfs-on-luks.nix` (ZFS-on-LVM-on-LUKS plumbing: the initrd `activate-pool` unit, `forceImportRoot = false`, scrub/TRIM/ZED, smartd, sanoid retention — each host's `system/zfs.nix` is then just its ARC cap and scrub window), and `impermanence-root.nix` (the tmpfs-root core: `/persist` `neededForBoot`, machine-id + SSH host keys, `/var/log`, `/home/irene` ownership, the `nix.envVars.TMPDIR` redirect off tmpfs, and the soft-reboot `persist-files` fixup — each host's `system/persistence.nix` then lists only its own service state).
- `hardware/` — opt-in hardware modules (audio, bluetooth, framework, tlp). Hosts pick what they need. (`pipewire.nix` was deleted — `audio.nix` supersedes it and `pipewire.nix` still referenced the renamed `hardware.pulseaudio` option. `zfs.nix` was deleted too: it only ever served orchid, hard-coding a two-NVMe smartd list and a `zfs_unstable` pin, and both ZFS hosts now share `modules/zfs-on-luks.nix` instead.)
...
- `desktop/` — editor/terminal/app configs consumed from Home Manager (vscode, helix, neovim, zed, tmux, fonts, home-packages, warp, mimeapps). `cli-packages.nix` is the CLI half of the package set, shared with the ocelot guest; `home-packages.nix` imports it and adds only the GUI/desktop/host-hardware sections, so the union tempest and orchid install is what it always was. A new package belongs in `cli-packages.nix` unless it needs a display or this machine's hardware. Present but imported by nothing: `desktop/emacs/`, `desktop/neovim/`, `desktop/warp.nix` and `desktop/orchid-gaming-packages.nix` (the three DE modules from the Plasma removal have since been deleted). `homes/tempest/default.nix` lists the deliberately-unimported set at the top of its `imports`.
- `rices/{estradiol,ember}` — desktop environments. Each rice has a `system.nix` (imported by the host) and a `default.nix` / home-manager-side files (imported by the home config). `estradiol` is currently imported by nothing — it was orchid's, and orchid is headless for now. `tempest` uses `ember`, which is **one rice with two compositor layers** under `rices/ember/compositors/{niri,mango}/` — both are installed and the session is chosen at the greeter per login; see `docs/adr/0012-one-rice-two-compositors.md`.
...
- Secrets go through `sops-nix` (an input; no home config imports it today) or out-of-band files under `/persist`. Don't commit raw secrets.
- Host `specialArgs` / `extraSpecialArgs` inject `inputs` and `hostname` — modules expect these available.
- Nix formatting: 2-space indent (alejandra). Shell scripts use explicit `set -euo pipefail` where relevant, or plain `#!/bin/sh` for simple wrappers (e.g. `build-vm`, `disk-install`).
